# backend/app/crud/global_chat.py
...
# use your existing cache_global
...
def create_system_join_message(db: Session, username: str) -> Message:
    system_msg = Message(
...
def mark_user_active(db: Session, user_id: int) -> None:
...
def list_active_users_window(db: Session, seconds: int = 11) -> list[dict]:
...
def poll_new_messages(db: Session, user_id: int) -> list[MessageOut]:

    with message_cache_lock:
        last_seen_msg_id, last_active = last_seen_msg.get(user_id, (0, get_current_time()))
...
def create_user_message(db: Session, msg: MessageIn) -> Message | None:
    ...
    with message_cache_lock:
        last_seen_msg_id, last_active = last_seen_msg.get(db_user.id, (0, get_current_time()))
